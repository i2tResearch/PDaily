...
@NamedQuery(name = LevodopaSchedule.findByPatientAsReadView, query = "SELECT new co.icesi.pdaily.events.levodopa.schedule.domain.model.view.LevodopaScheduleReadView(ls.id.id, ls.patientId.id, ls.medicineId.id, lm.name.name, lm.dose.value, lt.id.id, lt.label.name, ls.schedule.schedule, ls.pillsDose.dose) "
		+
...
@NamedQuery(name = LevodopaSchedule.findByIdAsReadView, query = "SELECT new co.icesi.pdaily.events.levodopa.schedule.domain.model.view.LevodopaScheduleReadView(ls.id.id, ls.patientId.id, ls.medicineId.id, lm.name.name, lm.dose.value, lt.id.id, lt.label.name, ls.schedule.schedule, ls.pillsDose.dose) "
		+
		"FROM LevodopaSchedule ls " +
		"INNER JOIN LevodopaMedicine lm ON lm.id = ls.medicineId " +
		"INNER JOIN LevodopaType lt ON lt.id = lm.typeId " +
