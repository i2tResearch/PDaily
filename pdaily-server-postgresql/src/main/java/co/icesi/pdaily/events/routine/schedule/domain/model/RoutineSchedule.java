package co.icesi.pdaily.events.routine.schedule.domain.model;
...
@NamedQuery(name = RoutineSchedule.findByPatientAsReadView, query = "SELECT new co.icesi.pdaily.events.routine.schedule.domain.model.view.RoutineScheduleReadView( rs.id.id, rs.patientId.id, rt.id.id, rt.label.text, rs.schedule.schedule ) "
		+
		"FROM RoutineSchedule rs " +
		"INNER JOIN RoutineType rt ON rs.type = rt.id " +
		"WHERE rs.tenant = :company " +
		"AND rs.patientId = :patientId")
@NamedQuery(name = RoutineSchedule.findByIdAsReadView, query = "SELECT new co.icesi.pdaily.events.routine.schedule.domain.model.view.RoutineScheduleReadView( rs.id.id, rs.patientId.id, rt.id.id, rt.label.text, rs.schedule.schedule ) "
		+
		"FROM RoutineSchedule rs " +
		"INNER JOIN RoutineType rt ON rs.type = rt.id " +
		"WHERE rs.tenant = :company " +
