...
@NamedQuery(name = AnimicEvent.findByPatientAsReadView, query = "SELECT new co.icesi.pdaily.events.animic.domain.model.view.AnimicEventReadView( a.id.id, a.patientId.id, a.type.id, t.label.name, t.intensity.intensity, a.ocurrenceDate.date ) "
		+
		"FROM AnimicEvent a INNER JOIN AnimicType t ON t.id = a.type WHERE a.patientId = :id AND a.tenant = :company")
@NamedQuery(name = AnimicEvent.findByIdAsReadView, query = "SELECT new co.icesi.pdaily.events.animic.domain.model.view.AnimicEventReadView( a.id.id, a.patientId.id, a.type.id, t.label.name, t.intensity.intensity, a.ocurrenceDate.date ) "
		+
