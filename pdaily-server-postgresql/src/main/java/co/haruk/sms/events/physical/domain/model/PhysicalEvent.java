package co.haruk.sms.events.physical.domain.model;
...
@NamedQuery(name = PhysicalEvent.findByPatientAsReadView, query = "SELECT new co.haruk.sms.events.physical.domain.model.view.PhysicalEventReadView(p.id.id, i.id.id, i.name.name, p.initialDate.date, p.finalDate.date, p.intensity.intensity) "
		+
