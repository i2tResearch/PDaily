package co.haruk.sms.events.physical.domain.model.detail;
...
import co.haruk.sms.events.physical.body.domain.model.BodyPartId;
import co.haruk.sms.events.physical.domain.model.PhysicalEvent;

@Entity
@Table(name = "event_physical_body_details")
@NamedQuery(name = BodyPartDetail.findByEventAsReadView, query = "SELECT new co.haruk.sms.events.physical.domain.model.view.BodyPartDetailReadView(b.id.id, p.id.id, p.name.name) "
		+
