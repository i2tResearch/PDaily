...
@NamedQuery(name = CustomerBusinessView.findByIdAsReadView, query = "SELECT new co.haruk.sms.business.structure.customer.domain.model.view.CustomerBusinessReadView(v.id.customerId.id,r.id.id,b.id.id,b.name.name,z.id.id,z.name.name) "
...
@NamedQuery(name = CustomerBusinessView.findForCustomerAsReadView, query = "SELECT new co.haruk.sms.business.structure.customer.domain.model.view.CustomerBusinessReadView(v.id.customerId.id,r.id.id,b.id.id,b.name.name,z.id.id,z.name.name) "
		+ "FROM CustomerBusinessView v " +
		"INNER JOIN BusinessUnit b ON v.id.businessUnitId = b.id " +
		"INNER JOIN SalesRep r ON r.id = v.salesRepId " +
		"LEFT JOIN Zone z ON z.id = v.zoneId " +
...
@NamedQuery(name = CustomerBusinessView.findByCustomerAndSalesRep, query = "SELECT v FROM CustomerBusinessView  v WHERE v.tenant = :company AND v.salesRepId = :salesRepId AND v.id.customerId = :customerId")
public class CustomerBusinessView extends PdailyTenantEntity<BusinessViewId> {
	...
	public static final String findByCustomerAndSalesRep = PREFIX + "findByCustomerAndSalesRep";
