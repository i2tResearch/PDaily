...
@NamedQuery(name = City.findByIdTextView, query = "SELECT new co.haruk.sms.business.structure.geography.domain.model.view.CityTextView(c.id.id,c.name.name,s.id.id,s.name.name,co.id.id,co.name.name) FROM City c INNER JOIN State s ON c.state = s.id INNER JOIN Country co ON s.country = co.id WHERE c.id = :cityId")
