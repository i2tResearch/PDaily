...
import co.haruk.sms.business.structure.customer.contact.infrastructre.persistence.ContactRepository;
...
public class ContactAppService {
	...
	public List<ContactReadView> findSalesContactsFor(String customerId) {
		var allContacts = repository.findSalesContactsFor( CustomerId.of( customerId ) );
		for ( ContactReadView contact : allContacts ) {
			contact.mainAddress = addressService.findMainAddressFor( ContactId.of( contact.id ) ).orElse( null );
		}
		return allContacts;
