package co.icesi.pdaily.security.user;
...
class UserResourceIT implements IDataSetDependent {
	...
	@Test
	@DisplayName("Fails if deleting doctor user")
	void failsIfDeletingDoctorUser() {
		given().delete( "/subscription/user/{0}", DoctorTesting.DOCTOR_ID )
				.then()
				.statusCode( 400 )
				.body(
						"messages[0]", containsStringIgnoringCase( "asignado como medico" )
				);
	}
